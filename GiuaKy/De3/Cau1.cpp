...
// Ước số nguyên tố M gần N nhất (ưu tiên nhỏ hơn nếu hòa)
...
// Các số nguyên tố trong khoảng [a, b]
vector<long long> primesInRange(long long a, long long b)
{
...
int main()
{
    ...
    vector<long long> primes = primesInRange(1000, M);
    long long sumPrimes = 0;
    cout << "3. Cac so nguyen to trong khoang [1000, M = " << M << "]: ";
    for (long long i = 0; i < (long long)primes.size(); i++)
    {
        cout << primes[i];
        if (i < (long long)primes.size() - 1)
            cout << ", ";
        sumPrimes += primes[i];
    }
    cout << endl;
